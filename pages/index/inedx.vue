...
<template>
  <view>
    ...
    <button color="primary" @click="onClickiOSMpay">iOS MPay</button>
    <button color="primary" @click="onClickAndroidMpay">Android MPay</button>
      ...
      >
        <view class="image">
          ...
          <view class="cu-bar bg-shadeBottom">
            <text class="text-cut">{{ item.type }}</text></view
          >
        </view>
...
export default {
  ...
  methods: {
    ...
    onClickiOSMpay() {
      ...
      uni.request({
        success: (response) => {
          console.log(response.data.preSignString);
          mpay.mpay(
            {
    ...
    goCategorieslist: function (e) {
      ...
      if (
        e.currentTarget.dataset.mid == 1 ||
        e.currentTarget.dataset.mid == 2
      ) {
        uni.navigateTo({
